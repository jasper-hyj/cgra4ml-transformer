...
def export_inference(model, hw, batch_size=1):
    ...
    for b in BUNDLES:
        ...
        b.cat_src_ibs = []   # reset concat-source tracking so re-exports are idempotent
    BUNDLES.clear()
    ...
    with open (f'./config_fw.h', 'w') as ch:
        ...
        for ib, b in enumerate(BUNDLES):
            ...
            n_cat = len(b.cat_src_ibs)
            cat_ib = (b.cat_src_ibs + [-1, -1, -1, -1])[:4]   # pad to fixed length 4
            ch.write(     f".is_bias={1*(b.core.b is not None):<3}, .is_pool={1*(b.pool is not None):<3}, .is_flatten={1*(b.flatten is not None):<3}, .is_softmax={1*(b.softmax is not None):<3}, .transpose_w_src={1 if (b.transpose_w_src and b.w_src_ib is not None) else 0:<3}, ")
            ch.write(     f".is_concat={1 if n_cat > 0 else 0:<3}, .n_concat_srcs={n_cat:<3}, .concat_src_ib_0={cat_ib[0]:<3}, .concat_src_ib_1={cat_ib[1]:<3}, .concat_src_ib_2={cat_ib[2]:<3}, .concat_src_ib_3={cat_ib[3]:<3}, ")
